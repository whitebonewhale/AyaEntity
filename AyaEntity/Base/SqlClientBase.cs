...
namespace AyaEntity.Base
{
  ...
  public abstract class SqlClientBase
  {
    ...
    public T Get<T>(SelectStatement sql = null)
    {
      ...
      return this.Connection.QueryFirst<T>(sql.Build(), sql.SqlParameters);
    }


    public IEnumerable<T> GetList<T>(SelectStatement sql = null)
    {
      if (sql == null)
      {
        sql = new SelectStatement()
      ...
      return this.Connection.Query<T>(sql.Build(), sql.SqlParameters);
    }
